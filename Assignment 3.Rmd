...
```
  ...
  Ans :  median then combine Auto and mpg01
...
```
  ...
  Ans : There is no realtionship b/w `mpg01` has a negative association with `Weight`, `Cylinders`, `Displacement`, and `Horsepower`.
...
```
  Ans : Using this predictors (`cylinders`, `weight`, `displacement`, and `horsepower`) a Logistic Regression Analysis the test error rate is 12.08791%.
